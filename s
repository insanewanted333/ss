...
local main = Library:Init {
	...
	name = ""..game:GetService("MarketplaceService"):GetProductInfo(17723040340).Name;
...
local tab = main:Tab({name = "Home"})

local tabl = main:Tab({name = "local"})

local tab1 = main:Tab({name = "Target"})

local tab3 = main:Tab({name = "Anti's"})

local tab4 = main:Tab({name = "Dev"})

local tab5 = main:Tab({name = "info"})
...
local premiumPlayers = { "x", "plr2", "plr3" }
local rankedPlayers = {"x", "", ""}
local owner = {"dexstr08"}
...
local rank
if isInList(player.Name, rankedPlayers) then
    rank = "Ranked"
elseif isInList(player.Name, premiumPlayers) then
    rank = "Premium"
elseif isInList(player.Name, owner) then
	rank = "Owner"
else
    rank = "Normal"
end

local xaa = tab:welcome({
    welc = "مرحبا بك",
    playeruser = player.Name,
    playername = player.DisplayName,
    img = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. player.UserId .. "&width=89&height=89&format=png",
    rank = rank,
    version = "2.5"
})
...
local Labelss = tabl:Void({ 
	name = "xn",
})
--


local tog = tabl:Toggle({
	callback = function(v)
...
local trgt = tab1:Target({
    callback = function(sex)
        for _, v in pairs(game.Players:GetPlayers()) do
            if string.sub(string.lower(v.Name), 1, string.len(sex)) == string.lower(sex) or
               string.sub(string.lower(v.DisplayName), 1, string.len(sex)) == string.lower(sex) then
               
                TargetV = v  -- تعيين اللاعب المستهدف إلى المتغير العام
                
                local userId = v.UserId
                local imageUrl = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. userId .. "&width=420&height=420&format=png"

                -- تحديث الواجهة
                game:GetService("CoreGui").MyLibrary.Main.ContentContainer:GetChildren()[3].Target.ImageLabel.Image = imageUrl
                game:GetService("CoreGui").MyLibrary.Main.ContentContainer:GetChildren()[3].Target.TextBox.Text = v.DisplayName
                game:GetService("CoreGui").MyLibrary.Main.ContentContainer:GetChildren()[3].Target.TextLabel.Text = "@" .. v.Name

                print("Target found: " .. v.DisplayName .. " (" .. "@" .. v.Name .. ")")
                break
            end
        end
    end
...
local btn = tab1:Button({
    name = "Goto",
    callback = function()
        if TargetV and TargetV.Character and TargetV.Character:FindFirstChild("HumanoidRootPart") then
...
local Label = tab3:Lable({ 
	name = "مضادات ضد كلبشة و بانج",
})
--
-- ✅ Toggle antihandcuff
local sitToggle = tab3:Toggle({
	title = "Anti HandCuff",
    callback = function(v)
...
local xs = tab4:Devs({
	...
	title1 = "s256brf95rib9dtt6b",

    img = "https://www.roblox.com/headshot-thumbnail/image?userId=12982577&width=89&height=89&format=png"
})


local Labelsxs = tab4:enreason({ 
	title1 = "Enjoy add me if u want something",
	title = "Discord : xwanted333.exe"
})
...
local Labelsxs = tab5:reason({ 

	title1 = "Enjoy                           ",

	title = "                     يشتغل بس عل ماب واحد : 🎙️ تجمع العرب / مايك"
...
-- Remove existing "open" UI if it exists
for i = 1, 5 do
    local openUI = game:GetService("CoreGui"):FindFirstChild("open")
    if openUI then
        openUI:Destroy()
    end
end
...
ImageButton.Position = UDim2.new(0.5, -29.5, 0.3, -24.5) -- Centered for all screen sizes
ImageButton.Size = UDim2.new(0, 59, 0, 49)
ImageButton.Image = "rbxassetid://14154325704"

UICorner.Name = "MainCorner"
UICorner.CornerRadius = UDim.new(0, 9)
UICorner.Parent = ImageButton

-- Toggle visibility of "MyLibrary.Main" when clicking the button
ImageButton.MouseButton1Down:Connect(function()
    local myLibrary = game:GetService("CoreGui"):FindFirstChild("MyLibrary")
    if myLibrary and myLibrary:FindFirstChild("Main") then
        myLibrary.Main.Visible = not myLibrary.Main.Visible
    else
        warn("⚠️ MyLibrary.Main not found!")
    end
end)
